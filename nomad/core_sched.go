...
func (c *CoreScheduler) forceGC(eval *structs.Evaluation) error {
	...
	// If we have ACLs enabled, trigger the local token GC process. Only if we
	// are the authoritative region, trigger the global token GC process.
	if c.srv.config.ACLEnabled {
		if err := c.expiredACLTokenGC(eval, false); err != nil {
			return err
		}
		if c.srv.config.AuthoritativeRegion == c.srv.Region() {
...
func (c *CoreScheduler) expiredACLTokenGC(eval *structs.Evaluation, global bool) error {
	...
	expiryThreshold := c.getThreshold(eval, "expired_acl_token",
	...
	for raw := expiredIter.Next(); raw != nil; raw = expiredIter.Next() {
		...
		// Check whether the token is old enough to be deleted and skip if it
		// is not.
		if token.CreateIndex > expiryThreshold {
			continue
		}
