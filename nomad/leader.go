...
func (s *Server) schedulePeriodic(stopCh chan struct{}) {
	...
	// Set up the expired ACL local token garbage collection ticker.
	localTokenExpiredGC := time.NewTicker(s.config.ACLTokenExpirationGCInterval)
	defer localTokenExpiredGC.Stop()
...
func (s *Server) schedulePeriodicAuthoritative(stopCh chan struct{}) {
	...
	// Set up the expired ACL global token garbage collection ticker.
	globalTokenExpiredGC := time.NewTicker(s.config.ACLTokenExpirationGCInterval)
	defer globalTokenExpiredGC.Stop()
